...
void GameLoop(ALLEGRO_EVENT ev)
{
	...
	while(!sair)
	{
        ...
        //Exibe fundo
				...
				if(complete)
				{
				al_draw_text(game.fonte_logo, al_map_rgb(0, 0, 0), 400, 80, ALLEGRO_ALIGN_CENTRE, get_idioma("Complete"));
				//TextBoxLoad(&ev, get_idioma("Welcome to LogiKid"));
				al_draw_text(game.fonte_menu, al_map_rgb(0, 0, 0), 400, 500, ALLEGRO_ALIGN_CENTRE, get_idioma("Press Enter to Continue..."));
				}
				if (game.level == 0)
				{
